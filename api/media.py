...
def upload_image(client, file_path):
    ...
    # Convert SVG to PNG for WordPress compatibility
...
def get_existing_media(client, filename):
    ...
    Prevents duplicate uploads on re-run.
    """
    # For SVG, search for PNG equivalent
...
def push_image_fields(client, parsed_output, images_dir="images", page_id=5):
    ...
    Uploads the image file first, then pushes the attachment ID to ACF.

    Args:
        client:        WPClient instance
        parsed_output: full dict from parse_document()
        images_dir:    directory containing image files
        page_id:       WordPress page ID for flat sections

    Returns list of result dicts with section, field, filename, attachment_id.
    """
